...
namespace CAE.Report
{
    ...
    /// 人员支出汇总表
    /// </summary>
    public class RYZCHZB:BaseReport
    {
        //public string colChar = "&nbsp&nbsp&nbsp";//用于显示分级用       
        public RYZCHZB(string key)
            : base(key)
        {
        ...
        {
            this.SqlFormat = "select bgcode.guid,bgcode.pguid,bgcode.bgcodekey,bgcode.bgcodename,item1.totalbg1 as totalbg1,item2.totalbg2 as totalbg2 from ss_bgcode bgcode  "
                            +"left join ( "
			                ...
			                +"            and departmentkey in ({0}) and bgtypekey='01' and bgstepkey in ({1}) "
			                +"            ) "
	                        ...
	                        +"    and guid_bg_main in( "
			                +"            select guid from bg_mainview where isnull(invalid,0)=1 "
			                +"            and departmentkey in ({0}) and bgtypekey='02' and bgstepkey in ({1}) "
			                +"            ) "
	                        +"    and bgyear='{2}' group by guid_bgcode) item2 on bgcode.guid=item2.guid_bgcode "
                            + "where bgcode.isstop=0 and bgcodekey not like '05%' order by bgcodekey ";
           this.tempalte=Path.Combine(this.tempalte, "ryzchzb.xls");        
      }
        ...
        public string GetSql(BBSearchCondition conditionModel)
        {
            ...
            if (!string.IsNullOrEmpty(conditionModel.treeModel) && (conditionModel.treeValue != null && conditionModel.treeValue != Guid.Empty))
            {
                switch (conditionModel.treeModel.ToLower())
                {
                    case "ss_department":
                        ...
                        departId = GetByDepartmentKey(conditionModel.treeValue.ToString());
                        break;
                    case "bg_setup":
                        if (conditionModel.treeValue == null)
                        {
                ...
                foreach (TreeNode item in conditionModel.TreeNodeList)
                {
                    switch (item.treeModel.ToLower())
                    {
                        case "ss_department":
                            ...
                            departId = GetByDepartmentKey(item.treeValue);
                            break;
                        case "bg_setup":
                            if (item.treeValue == null)
                            {
            ...
            {
                departId = GetByDepartmentKey(Guid.Empty.ToString());
            }
        ...
        public string GetExportPath(DataTable data, out string fileName, out string message, ReportHeadModel model)
        {
            ...
            try
            {
                ...
                string filePath = ExportExcel.Export(data, this.tempalte, 4, 0, new List<ExcelCell>() { new ExcelCell(1, 1, model.DepartmentName), new ExcelCell(1, 2, model.Year), new ExcelCell(4, 2, model.RMBUnit) });
