...
namespace CAE.Report
{
    ...
    /// 项目支出科目汇总表
    /// </summary>
    public class XMZCKMHZB:BaseReport
    {
        public string colChar = "&nbsp&nbsp&nbsp";//用于显示分级用       
        public XMZCKMHZB(string key)
            : base(key)
        {
        ...
        public override void Init()
        {
            this.SqlFormat = "select bgcode.guid,bgcode.pguid,bgcode.BGCodeKey,bgcode.BGCodeName,item1.totalbg1 as totalbg1,item2.totalbg2 as totalbg2 from ss_bgcode bgcode "
                          +"left join(select guid_BGcode,sum(total_BG) as totalbg1 from bg_detailview where "
                          ...
                          +") and bgyear='{2}' "
                          +"group by guid_bgcode) item1 on bgcode.guid=item1.guid_bgcode "
                          +"left join( "
		                  ...
		                  +"           and departmentkey in ({0}) and bgtypekey ='02' and bgstepkey in ({1}) "
                          +") and bgyear='{2}' "
                          +"group by guid_bgcode) item2 on bgcode.guid=item2.guid_bgcode "
                          +"where bgcode.BGCodeKey not like '05%' and bgcode.isstop=0 order by bgcode.BGCodeKey";
           this.tempalte=Path.Combine(this.tempalte, "xmzckmhzb.xls");        
      }
