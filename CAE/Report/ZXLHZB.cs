...
namespace CAE.Report
{
    ...
    public class ZXLHZB:BaseReport
    {
        ...
        public override void Init()
        {
           ...
           + "  from bg_detailview "
           + "  where bgyear='{0}' "//Year
           + "  and guid_item in(select guid from bg_item where bgitemkey in ('02','04','08')) "
           + "  and guid_bg_main in(select guid from bg_mainview where isnull(Invalid,0)=1 "
           + "  and GUID_FunctionClass in ({1}) and bgstepkey in ({2})) "//{1}departmentGUID {2} BGStepKey
           + "  group by guid_bgcode) b on a.guid=b.guid_bgcode "
           + " left join "
           + " ( "
           + " select guid_bgcode,sum(total_bx) as totalbx from bx_detailview where GUID_PaymentNumber in(  SELECT guid FROM dbo.CN_PaymentNumber WHERE GUID_FunctionClass  in ({1}))  "//departmentGUID
           ...
           + " and guid_paymentnumber in(select guid from cn_paymentnumberview where isnull(projectkey,'')='' and isnull(isguoku,0)=1 "
           + " ) "
        ...
        private string GetByDepartmentId(string guid)
        {
            ...
            var depList1 = new List<Guid>();
            if (string.IsNullOrEmpty(guid) || guid == Guid.Empty.ToString())
            {
                IntrastructureFun dbobj = new IntrastructureFun();
                var depList = dbobj.GetFunctionClassView();
                if (depList!=null&&depList.Count > 0) {
                    depList1 = depList.Select(e => e.GUID).ToList();
                }
                depId = depList1.GetStrGUIDS();
            }
            else
            {
                depId = "'"+guid+"'";
