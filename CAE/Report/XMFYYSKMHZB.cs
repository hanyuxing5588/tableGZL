...
namespace CAE.Report
{
    ...
    public string DepName { get; set; }//报表导出用
        ...
        {   
            //0 部门编号 1预算 2 年度
            this.SqlFormat = "  select bgcode.BGCodeKey,bgcode.BGCodeName,item1.totalbg1 as totalbg1,item2.totalbg2 as totalbg2 from ss_bgcode bgcode left join(select guid_BGcode,sum(total_BG) as totalbg1 from bg_detailview " +
             " where guid_item in(select guid from bg_item where bgitemkey in ('04','21','24')) and " +
              "   guid_bg_main in(select guid from bg_mainview where isnull(invalid,0)=1 " +
              "and departmentkey in ({0}) and bgtypekey ='02'  and bgstepkey in ({1})) and bgyear={2} group by guid_bgcode) item1 " +
              "on bgcode.guid=item1.guid_bgcode left join(select guid_BGcode,sum(total_BG) as totalbg2 from bg_detailview where guid_item in(select guid from bg_item where bgitemkey='08') " +
              "and guid_bg_main in(select guid from bg_mainview where isnull(invalid,0)=1 and departmentkey in ({0}) and bgtypekey ='02' "+
            "and bgstepkey in ({1})) and bgyear={2} group by guid_bgcode) item2 on bgcode.guid=item2.guid_bgcode  order by bgcode.BGCodeKey";
        ...
        //        newRow["bgcodekey"] = bk.Length == 2 ? bk : this.colChar + bk;
        //        newRow["BGCodeName"] = row["BGCodeName"].ToString();
        //        double d = 0;
        //        newRow["totalbg1"] = double.TryParse((row["totalbg1"] + "").ToString(), out d) ? FormatMoney(1, d/unit) : "0.00" ;
        //        dhj += d;
