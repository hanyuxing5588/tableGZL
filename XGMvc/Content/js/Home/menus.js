                ...
                if (fitem.Child.length != 0) {
                    $.each(fitem.Child, function (index, sitem) {
                        ...
                        if (sitem.Child.length != 0) {
                            var aTag = '<a id="{4}" title="{0}"  href="#" onclick="openTabs1(\'{0}\', \'{1}\', \'{2}\', \'{3}\', \'{4}\', \'{5}\')">{0}</a>';
                            var sTag = '<s class="info{4}" onclick="openTabs1(\'{0}\', \'{1}\'), \'{2}\', \'{3}\', \'{4}\', \'{5}\'"></s>';
                            strPanelContent=$.format('<div id="{0}">', son.MenuKey);
                            $.each(sitem.Child, function (index, gitem) {
                                ...
                                strPanelContent = strPanelContent + [
                                          index % 3 == 0 ? '<dl class="fore">' : '',
                                            $.format('<dt class="iconcls{4}" onclick="openTabs1(\'{0}\', \'{1}\', \'{2}\', \'{3}\', \'{4}\',\'{5}\')">', gitem.MenuName, Scope, "", '1', gitem.MenuKey, gitem.GUID_Auth+""),
                                                '<em>',
                                                    $.format(aTag, gitem.MenuName, Scope, "", '1', gitem.MenuKey, gitem.GUID_Auth + ""),
                                                    $.format(sTag, gitem.MenuName, Scope, "", '1', gitem.MenuKey, gitem.GUID_Auth + ""),
                                                '</em>',
                                            '</dt>',
                                       index % 3 == 2 ? '</dl>' : ''
                                ].join('');
                            });
                            strSon += strPanelContent + '</div>';
                        } else {
                            strSon = strSon + [
...
//新增tab标签 for 穿透到单据列表
var openTabs2 = function (title, scope, guid, status,url) {
    var scope = $.trim(scope);
    if (scope != undefined && scope.length >= 0 && scope != "") {
        var existIndex = existsTab($('#tt'), title, guid);
        if (existIndex >= 0) {
            $('#' + scope).attr('src', url); //hanyx
            $('#tt').tabs('select', existIndex);
            return;
        }
        var content = '<iframe id="' + scope + '" scrolling="no" frameborder="0" src="' + url + '" style="width:100%;height:99%;"></iframe>';
        var subTitle = title;
        if (title.length > 6) {
...
//新增tab标签
var openTabs1 = function (title, scope, guid, status, key,GUID_Auth) {
    var scope = $.trim(scope);
    if (scope != undefined && scope.length >= 0 && scope != "") {
        var existIndex = existsTab($('#tt'), title, guid);
        if (existIndex >= 0) {
            $('#' + scope).attr('src', GetUrl1(scope, guid, status, GUID_Auth)); //hanyx
            $('#tt').tabs('select', existIndex);
            return;
        }
        var content = '<iframe id="' + scope + '" scrolling="no" frameborder="0" src="' + GetUrl1(scope, guid, status, GUID_Auth) + '" style="width:100%;height:99%;"></iframe>';
        var subTitle = title;
        if (title.length > 6) {
            subTitle = title.substring(0, 4) + "..";
        }
...
//得到href
function GetUrl1(scope, guid, status, GUID_Auth) {
    var url = "";
    if (scope != undefined && scope.length >= 0 && scope != "") {
        if (GUID_Auth) {
            url = $.format("/{0}?scope={1}&CommonFunGuid={2}", scope, scope, GUID_Auth);
        } else {
