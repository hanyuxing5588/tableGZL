...
function addToFavorite() {
    ...
    document.all ? window.external.AddFavorite(a, b) : window.sidebar ? window.sidebar.addPanel(b, a, "") : alert("\u5bf9\u4e0d\u8d77\uff0c\u60a8\u7684\u6d4f\u89c8\u5668\u4e0d\u652f\u6301\u6b64\u64cd\u4f5c!\n\u8bf7\u60a8\u4f7f\u7528\u83dc\u5355\u680f\u6216Ctrl+D\u6536\u85cf\u672c\u7ad9\u3002"),
    createCookie("_fv", "1", 30, "/;domain=jd.com")
}
function search(a) {
    var b = "http://search.jd.com/Search?keyword={keyword}&enc={enc}{additional}{area}",
    c = search.additinal || "",
    ...
    case 6:
        i = "-1",
        b = "http://music." + pageConfig.FN_getDomain() + "/8_0_desc_0_0_1_15.html?key={keyword}"
    }
    if ("string" == typeof h && "" != h && "string" == typeof i) {
        var m = /^(?:[1-8])?([1-3])$/;
        i = "-1" == i ? "": m.test(i) ? RegExp.$1: "";
        var n = g.replace(/\{level}/, i);
        n = n.replace(/\{cid}/g, h),
        c += n
    }
    "string" == typeof j && "" != j && (c += "&ev=" + j);
    var o = "";
    if (k) {
        var p = CookieUtil.getCookie("ipLoc-djd");
        if (null != p) {
            var q = p.split("-");
            if (q.length > 0) {
                var r = q[0];
...
function setWebBILinkCount(a) {
    try {
        if (a.length > 0) {
            ...
            b.src = "http://counter.360buy.com/aclk.aspx?key=" + a,
...
function reduceCompare(a) {
    ...
    var b = readCookie("compare"),
    c = b.indexOf(a),
    d = b.indexOf("|||", c) + 3,
    e = b.substring(c, d),
    f = b.replace(e, "");
    createCookie("compare", f),
    0 == proIDs.length && $(".comPro").hide()
}
...
function isCoo() {
    ...
    if (a) {
        for (var b = a.split("|||"), c = "", d = 0; d < b.length - 1; d++) c += "<li id='check_" + b[d].split("||")[0] + "'><a title='\u5220\u9664' class='close' onclick='reduceCompare(" + b[d].split("||")[0] + ")'></a>" + unescape(b[d].split("||")[1]) + "</li>",
        proIDs.push(b[d].split("||")[0]);
        $("#comProlist").html(c),
        $("#compare").show(),
        $(".comPro").show()
    }
...
function getRandomObj(a, b) {
    for (var c = 0,
    d = 0; d < a.length; d++) c += b[d] || 1,
    b[d] || b.push(1);
    for (var e = Math.ceil(Math.random() * c), f = [], g = [], d = 1; c + 1 > d; d++) f.push(d);
    for (var d = 0; d < a.length; d++) g[d] = f.slice(0, b[d]),
    f.splice(0, b[d]);
    for (var d = 0; d < g.length; d++) for (var h = 0; h < g[d].length; h++) if (e == g[d][h]) return a[d]
}
function setRandomAds(a, b, c, d) {
    var e, f = getRandomObj(a, b),
...
function sBuilder() {
    ...
    this.toString = function(a, b, c) {
        var a = a ? a: "",
        d = this.strings.join(a);
        return b && c && (d = d.substr(b, c)),
        d
    }
}
...
pageConfig.FN_InitSlider = function(a, b) {
    ...
    b.sort(c);
    var d, e, f, g, h = b[0].data,
    i = [],
    j = 3 == h.length ? "style2": "style1",
    k = screen.width < 1210;
    i.push('<div class="slide-itemswrap"><ul class="slide-items"><li class="'),
    i.push(j),
    i.push('" data-tag="'),
    i.push(b[0].aid),
    i.push('">');
    for (var l = 0; l < h.length; l++) d = h[l],
    e = k ? d.widthB: d.width,
    f = k ? d.heightB: d.height,
    g = k ? d.srcB: d.src,
    i.push('<div class="fore'),
    i.push(l + 1),
    i.push('"><a target="_blank" href="'),
    i.push(d.href),
    i.push('"><img data-img="2" src="'),
    i.push(g),
    i.push('" width="'),
    i.push(e),
    i.push('" height="'),
    ...
    i.push(d.alt),
    i.push('" /></a></div>');
    i.push('</li></ul></div><div class="slide-controls"><span class="curr">1</span></div>'),
    document.getElementById(a).innerHTML = i.join("")
};
var CookieUtil = {};
...
function (a) {
    a.fn.jdTab = function (b, c) {
        ...
        b || {}),
        e = a(this).find(d.tab).eq(0).find(d.itemTag),
        f = a(this).find(d.content);
        ...
        h = function (a, b) {
            d.subTimer = setTimeout(function () {
                k(),
                b ? (d.index++, d.index == e.length && (d.index = 0)) : d.index = a,
                d.type = null != e.eq(d.index).attr(d.source) ? "dynamic" : "static",
                j()
            },
            d.delay)
        },
        ...
        k = function () {
            e.eq(d.index).removeClass(d.currClass),
            f.eq(d.index).hide()
        };
        e.each(function (b) {
            a(this).bind(d.event,
            function () {
                return clearTimeout(d.subTimer),
                clearInterval(d.mainTimer),
                h(b, !1),
                !0
            }).bind("mouseleave",
            function () {
...
function (a) {
    a.fn.jdSlide = function (b) {
        ...
        n = function (a, b) {
            a == k && (a = 0),
            d = setTimeout(function () {
                g.find("span").eq(f.index).removeClass(f.current),
                g.find("span").eq(a).addClass(f.current),
                o(a, b)
            },
            20)
        },
        o = function (a, b) {
            var e = parseInt(h),
            l = Math.abs(e + f.index * f.height),
            m = Math.abs(a - f.index) * f.height,
            p = Math.ceil((m - l) / 4);
            l == m ? (clearTimeout(d), b ? (f.index++, f.index == k && (f.index = 0)) : f.index = a, i = !0, i && j && (clearTimeout(c), c = setTimeout(function () {
                n(f.index + 1, !0)
            },
            f.delay2))) : (f.index < a ? (h = e - p, g.find("ul").css({
                top: h + "px"
            })) : (h = e + p, g.find("ul").css({
                top: h + "px"
            })), i = !1, d = setTimeout(function () {
                o(a, b)
            },
            20))
        },
        p = function () {
            for (var a = [], b = 1; k > b; b++) a.push("<li><a href='"),
            a.push(f.pics[b].href),
            a.push("' target='_blank'><img src='"),
            a.push(f.pics[b].src),
            a.push("' width='"),
            a.push(f.width),
            a.push("' height='"),
...
pannel.gotop = {
    ...
    render: function(a) {
        var b, c, d = $(this.settings.element),
        e = $(this.settings.target),
        f = $.browser.scroll().top + $.browser.client().height - d.height() - 10 + "px";
        switch (a) {
        case 0:
            c = e.width() >= 1200 ? 1200 : 980,
            b = e.offset().left + c + "px",
            $.browser.isIE6 ? d.css({
                left: b,
                top: f
            }) : d.css({
...
//log = function(a, b) {
//    var c = "";
//    for (i = 2; i < arguments.length; i++) c = c + arguments[i] + "|||";
//    var d = loguri.replace(/\$type1\$/, escape(a));
//    d = d.replace(/\$type2\$/, escape(b)),
//    d = d.replace(/\$data\$/, escape(c)),
//    d = d.replace(/\$pin\$/, escape(decodeURIComponent(readCookie("pin")))),
//    d = d.replace(/\$referrer\$/, escape(document.referrer)),
//    $.getJSON(d, callback1);
