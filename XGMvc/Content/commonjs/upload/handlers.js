...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
    try {
        if (numFilesSelected > 0) {
            document.getElementById(this.customSettings.cancelButtonId).disabled = false;   //上传成功后取消按钮的编辑状态。true:不可编辑
        }
...
function uploadSuccess_zcfg(file, serverData) {
    try {
        ...
        progress.toggleCancel(false);
        var guid = '#zcfg-OA_FileTempModel-GUID_OA_FileTemp';
        var FileName = '#zcfg-OA_OfficeFile-FileName';
        for (var i = 0, j = IDS.length; i < j; i++) {
            //获取到之前赋的guid
            var result = '';
            //判断是不是最后一个，是最后一个就不加逗号，否则添加逗号
            if (i == (IDS.length - 1)) {
                result += IDS[i];
            } else {
                result += IDS[i] + ',';
...
function uploadSuccess_tzgg(file, serverData) {
    try {
        ...
        progress.setStatus("完成");
        progress.toggleCancel(false);
        var guid = '#tzgg-OA_FileTempModel-GUID_OA_FileTemp';
        var FileName = '#tzgg-OA_Notice-FileName';
        for (var i = 0, j = IDS.length; i < j; i++) {
            //获取到之前赋的guid
            var result = '';
            //判断是不是最后一个，是最后一个就不加逗号，否则添加逗号
