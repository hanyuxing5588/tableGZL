...
$(document).ready(function () {
    ...
    function loadData(id) {
        $.post('/JCjksz/GetBB', { id: id }, function (data) {
    ...
    var SetPRalation = function (type) {
        ...
        if (!row1 && type == 0) {
            $.messager.alert('系统提示', '请选择用友人员！');
            return;
        }
        var toformID = row1.citemcode;
        $.ajax({
            url: '/JCjksz/Savexmdygxsz',
            data: { id: id, toformKey: toformID, compasrMainID: node.id, type: type, Ctype: 'Department', ClassID: '2' },
            error: function () {
...
function loadu8project(node, record) {
    $("#projectGrid").datagrid('load', { "extdb": node.attributes.ExteriorDataBase, "year": node.attributes.ExteriorYear, "prj": "Department" });
};
