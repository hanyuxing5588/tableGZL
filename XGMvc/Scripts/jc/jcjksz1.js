...
$(document).ready(function () {
    ...
    function loadData(id) {
        $.post('/JCjksz/GetGYS', { id: id }, function (data) {
    ...
    var SetPRalation = function (type) {
        ...
        if (!treeNode) {
            $.messager.alert('系统提示', '请选择要对应的供应商');
            return;
        }
        ...
        if (!row1 && type == 0) {
            $.messager.alert('系统提示', '请选择用友供应商！');
            return;
        }
        var toformID = row1.citemcode;
        $.ajax({
            url: '/JCjksz/Savexmdygxsz',
            data: { id: id, toformKey: toformID, compasrMainID: node.id, type: type, Ctype: 'Vendor', ClassID: '17' },
            error: function () {
...
function loadu8project(node, record) {
    $("#projectGrid").datagrid('load', { "extdb": node.attributes.ExteriorDataBase, "year": node.attributes.ExteriorYear, "prj": "Vendor" });
};
