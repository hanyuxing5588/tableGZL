...
$.selectChildren = function (target, id, idField, deepCascade, status) {
    ...
    for (var i = 0; i < children.length; i++) {
        ...
        $.setCheckRowByField(children[i], childId, target);
        if (status) {
...
$.getHashTable = function (isChecked) {
    ...
    hs["IsBrowser"] = v;
    v = !isChecked ? false : $('#chkUpdate').attr('checked') == "checked";
    hs["IsModify"] = v;
    v = !isChecked ? false : $('#chkDelete').attr('checked') == "checked";
    hs["IsDelete"] = v;
...
$.setCheckRowByField = function (data, guid, t) {
...
$.initBind = function (e) {
    ...
    if (opts.cascadeCheck || opts.deepCascadeCheck || opts.threeLinkCheck) {
        ...
        $.setCheckRowByField(selectNode, id, target);
...
$.setCheckDisabled = function (guid, checkState, target) {
    var arr = "IsBrowser,IsModify,IsDelete,IsTimeLimited,IsDefault".split(',');
...
//点击有效期显示 编辑日期的控制
$.editingId = '';
$.onCheckIsTimeChange = function (Guid, field, isEdit, treeGrid, selectedRowNode) {
    if (field == 'IsTimeLimited') {
        if (!isEdit) {
            treeGrid.treegrid('endEdit', $.editingId);
            selectedRowNode["StartTime"] = '';
            selectedRowNode["StopTime"] = '';
            treeGrid.treegrid('refresh', Guid);
            //add
            ...
            selectedRowNode["StartTime"] = startDate;
            selectedRowNode["StopTime"] = endDate;
...
//        var selectNodePre = t.treegrid('getSelected');
       ...
       //如果选择，对应的权限CheckBox改为可用

        //如果选中的是使用有效期
        $.onCheckIsTimeChange(idAttr[1], idAttr[0], ch, t,selectNode);
...
$.gridCheckBoxCol = function (index, row, e1, field, e) {
    ...
    if ((row[field]+'').toLowerCase() == "true") {
        chk = $.format("<input id=\"{0}\" checked='{1}' disabled='disabled' type=\"checkbox\" onclick=\"javascript:$.OnCheckClick(this)\">", field  +'|'+ row.Guid, ch);
    } else {
...
$.getDataNodes = function (dataNode) {
    ...
    var getNodesO = function (dataNode) {
        for (var i = 0; i < dataNode.length; i++) {
            var node = dataNode[i];
            //            if (!node.IsBrowser && !node.IsModify && !node.IsDelete && !node.IsTimeLimited && !node.IsDefault) {
            //                continue;
            //            }
...
$(document).ready(function () {
    ...
    $('#auth-close').bind('click', function () {
        $.messager.confirm("提示", "正在编辑,是否退出?", function (e) {
    ...
    $('#auth-cancel').bind('click', function () {
        var target = $('#dwAuthTreeGrid');
        $.CheckOrUnCheckAll(target, false);
    });
    //Grid中的全选
