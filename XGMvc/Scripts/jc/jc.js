...
*/
$.extend($.view, {
    init: function (scope, status, dataguid) {debugger
        if (scope && status) {
...
$.extend($.fn.linkbutton.methods, {
    ...
    examine: function (jq) {//先获取当前tree
        ...
        //拿到所有的tree的Id
        var treeIdArr = opts.treeId;
        //拿到页面状态，如果为新建状态，那么直接返回，不做任何处理
        var pageState = $.view.getStatus(scope);
        if (pageState == "1") return true;
        //有多个tree的情况 
        for (var i = 0, j = treeIdArr.length; i < j; i++) {
            var currTreeId = '#' + treeIdArr[i];
            //得到tree中options的属性
            var currtreeOpts = $(currTreeId).tree('options');
            //要提示的指定tabs的名称
            var msg = currtreeOpts.msg;
            var scope = currtreeOpts.scope;
            //校验不成功的时候返回页面初始状态
            var retStatus = currtreeOpts.retStatus;
            if (retStatus == 1) return;
            if (currtreeOpts.checkbox == false) {   //没有checkbox复选框
                //判斷當前tree节点是否被选中，节点是否被点击过
                if ($(currTreeId).tree('getSelected') == null) {
                ...
                if (!$(currTreeId).tree('getChecked').length) {
                    $.messager.alert('提示', '请选择要' + msgtext + '的' + msg + '！');
