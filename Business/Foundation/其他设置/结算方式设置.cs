...
{
    public class 结算方式设置 : BaseDocument
    {
        public 结算方式设置() : base() { }
        public 结算方式设置(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_SettleType().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_SettleType main = new SS_SettleType();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();

            this.InfrastructureContext.SS_SettleType.AddObject(main);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_SettleType.FirstOrDefault(e => e.GUID == g);
            main.Fill(jsonModel.m);
        ...
        {
            SS_SettleType main = this.InfrastructureContext.SS_SettleType.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_SettleType LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_SettleType data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SS_SettleType mModel = data;

            #region   主表字段验证
            //结算方式编号
            if (string.IsNullOrEmpty(mModel.SettleTypeKey))
            {
                str = "结算方式编号 字段为必输入项！";
            ...
            //结算方式名称
            if (string.IsNullOrEmpty(mModel.SettleTypeName))
            {
                str = "结算方式名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            SS_SettleType model = (SS_SettleType)data;
            //主Model验证
