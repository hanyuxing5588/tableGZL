...
{
    public class 收入类型设置 : BaseDocument
    {
        public 收入类型设置() : base() { }
        public 收入类型设置(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_SRType().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_SRType main = new SS_SRType();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();
            this.InfrastructureContext.SS_SRType.AddObject(main);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_SRType.FirstOrDefault(e => e.GUID == g);
            main.Fill(jsonModel.m);
        ...
        {
            SS_SRType main = this.InfrastructureContext.SS_SRType.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_SRType LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_SRType data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SS_SRType mModel = data;
            #region   主表字段验证
            //类型编号
            if (string.IsNullOrEmpty(mModel.SRTypeKey))
            {
                str = "类型编号 字段为必输入项！";
            ...
            //类型名称
            if (string.IsNullOrEmpty(mModel.SRTypeName))
            {
                str = "类型名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            SS_SRType model = (SS_SRType)data;
            //主Model验证
