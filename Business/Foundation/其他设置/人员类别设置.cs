...
{
    public class 人员类别设置 : BaseDocument
    {
        public 人员类别设置() : base() { }
        public 人员类别设置(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_PersonType().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_PersonType main = new SS_PersonType();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();
            var resu = jsonModel.m.GetValueByAttribute("IsStop");
            var b = resu.ToString().ToLower() == "false" ? main.IsStop = 0 : main.IsStop = 1;
            this.InfrastructureContext.SS_PersonType.AddObject(main);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_PersonType.FirstOrDefault(e => e.GUID == g);
            var resu = jsonModel.m.GetValueByAttribute("IsStop");
            var b = resu.ToString().ToLower() == "false" ? main.IsStop = 0 : main.IsStop = 1;
            main.Fill(jsonModel.m);
        ...
        {
            SS_PersonType main = this.InfrastructureContext.SS_PersonType.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_PersonType LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_PersonType data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SS_PersonType mModel = data;

            #region   主表字段验证
            //人员类别编号
            if (string.IsNullOrEmpty(mModel.PersonTypeKey))
            {
                str = "人员类别编号 字段为必输入项！";
            ...
            //人员类别名称
            if (string.IsNullOrEmpty(mModel.PersonTypeName))
            {
                str = "人员类别名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            SS_PersonType model = (SS_PersonType)data;
            //主Model验证
