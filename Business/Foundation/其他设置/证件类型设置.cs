...
{
    public class 证件类型设置 : BaseDocument
    {
        public 证件类型设置() : base() { }
        public 证件类型设置(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_CredentialType().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_CredentialType main = new SS_CredentialType();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();

            this.InfrastructureContext.SS_CredentialType.AddObject(main);
            this.InfrastructureContext.SaveChanges();
            return (Guid)main.GUID;
        }
        /// <summary>
        /// 修改
        /// </summary>
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_CredentialType.FirstOrDefault(e => e.GUID == g);
            main.Fill(jsonModel.m);
        ...
        {
            SS_CredentialType main = this.InfrastructureContext.SS_CredentialType.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_CredentialType LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_CredentialType data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SS_CredentialType mModel = data;

            #region   主表字段验证
            //证件编号
            if (string.IsNullOrEmpty(mModel.CredentialTypekey))
            {
                str = "证件编号 字段为必输入项！";
            ...
            //证件名称
            if (string.IsNullOrEmpty(mModel.CredentialTypeName))
            {
                str = "证件名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            SS_CredentialType model = (SS_CredentialType)data;
            //主Model验证
