...
{
    public class 交通工具 : BaseDocument
    {
        public 交通工具() : base() { }
        public 交通工具(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_Traffic().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_Traffic main = new SS_Traffic();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();

            this.InfrastructureContext.SS_Traffic.AddObject(main);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_Traffic.FirstOrDefault(e => e.GUID == g);
            main.Fill(jsonModel.m);
        ...
        {
            SS_Traffic main = this.InfrastructureContext.SS_Traffic.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_Traffic LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_Traffic data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SS_Traffic mModel = data;

            #region   主表字段验证
            //交通工具编号
            if (string.IsNullOrEmpty(mModel.TrafficKey))
            {
                str = "交通工具编号 字段为必输入项！";
            ...
            //交通工具名称
            if (string.IsNullOrEmpty(mModel.TrafficName))
            {
                str = "交通工具名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            SS_Traffic model = (SS_Traffic)data;
            //主Model验证
