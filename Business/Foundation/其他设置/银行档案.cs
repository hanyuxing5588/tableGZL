...
{
    public class 银行档案 : BaseDocument
    {
        public 银行档案() : base() { }
        public 银行档案(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_Bank().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_Bank main = new SS_Bank();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();

            this.InfrastructureContext.SS_Bank.AddObject(main);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_Bank.FirstOrDefault(e => e.GUID == g);
            main.Fill(jsonModel.m);
        ...
        {
            SS_Bank main = this.InfrastructureContext.SS_Bank.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_Bank LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_Bank data)
        {
            ...
            #region   主表字段验证
            //银行编号
            if (string.IsNullOrEmpty(mModel.BankKey))
            {
                str = "银行编号 字段为必输入项！";
            ...
            //支出类型名称
            if (string.IsNullOrEmpty(mModel.BankName))
            {
                str = "银行名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
