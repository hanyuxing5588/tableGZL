...
{
    public class 应收应付类型 : BaseDocument
    {
        public 应收应付类型() : base() { }
        public 应收应付类型(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_WLType().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_WLType main = new SS_WLType();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();
            this.InfrastructureContext.SS_WLType.AddObject(main);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_WLType.FirstOrDefault(e => e.GUID == g);
            main.Fill(jsonModel.m);
        ...
        {
            SS_WLType main = this.InfrastructureContext.SS_WLType.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_WLType LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_WLType data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SS_WLType mModel = data;
            #region   主表字段验证
            //往来类型编号
            if (string.IsNullOrEmpty(mModel.WLTypeKey))
            {
                str = "往来类型编号 字段为必输入项！";
            ...
            //往来类型名称
            if (string.IsNullOrEmpty(mModel.WLTypeName))
            {
                str = "往来类型名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            SS_WLType model = (SS_WLType)data;
            //主Model验证
