...
{
    public class 收款类型设置 : BaseDocument
    {
        public 收款类型设置() : base() { }
        public 收款类型设置(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SS_SKType().ModelName());
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            SS_SKType main = new SS_SKType();
            main.Fill(jsonModel.m);
            main.GUID = Guid.NewGuid();

            this.InfrastructureContext.SS_SKType.AddObject(main);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            main = this.InfrastructureContext.SS_SKType.FirstOrDefault(e => e.GUID == g);
            main.Fill(jsonModel.m);
        ...
        {
            SS_SKType main = this.InfrastructureContext.SS_SKType.FirstOrDefault(e => e.GUID == guid);
        ...
        /// <returns></returns>
        private SS_SKType LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SS_SKType data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SS_SKType mModel = data;

            #region   主表字段验证
            //收款类型编码
            if (string.IsNullOrEmpty(mModel.SKTypeKey))
            {
                str = "收款类型编码 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
            }
            //收款类型名称
            if (string.IsNullOrEmpty(mModel.SKTypeName))
            {
                str = "收款类型名称 字段为必输入项！";
                resultList.Add(new ValidationResult("", str));
            }
            //收入或者往来类型对象
            if (mModel.GUID_SRWLType.IsNullOrEmpty())
            {
                str = "收入或者往来类型对象ID不能为空！";
                resultList.Add(new ValidationResult("", str));
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            SS_SKType model = (SS_SKType)data;
            //主Model验证
