...
namespace Business.Reimbursement
{
    ...
    public class 借款单填制 : WLDocument    {

        public 借款单填制() : base() { }
        public 借款单填制(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel New()
        {
            try
            {
                ...
                jmodel.m = model.Pick();
               var attr=jmodel.m.Find(e => e.n == "DocDate");
               if (attr != null)
               {
                   attr.v = DateTime.Now.ToString("yyyy-MM-dd HH:MM:ss");
               }
        ...
        public override JsonModel Retrieve(Guid guid)
        {
            ...
            try
            {
                ...
                {
                    jmodel.m = main.Pick();
                    //jmodel.m.AddJsonAttrModel("ActionMemo", main.DocMemo + "", "WL_Main");
                    ...
                    //明细中f 填充默认值
                    WL_DetailView dModel = new WL_DetailView();
                    dModel.FillDetailDefault<WL_DetailView>(this, this.OperatorId);
                    JsonGridModel fjgm = new JsonGridModel(dModel.ModelName());
                    jmodel.f.Add(fjgm);
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(strMsg))
                {
                    ...
                    OperatorLog.WriteLog(this.OperatorId, value, status, "借款单填制", data);
            ...
            {
                OperatorLog.WriteLog(this.OperatorId, "借款单填制", ex.Message, data, false);
                result.result = JsonModelConstant.Error;
        ...
        private List<ValidationResult> VerifyResult_WL_Main(WL_Main data)
        {
            ...
            //用款日期进行验证
            //规则：大于12点后提前3天约款 否则提前两天约款
            //if(!mModel.DocDate.IsNullOrEmpty())
            //{
            //    int hour = DateTime.Now.Hour;
            //    TimeSpan ts = mModel.DocDate - DateTime.Parse(DateTime.Now.ToString("yyyy-MM-dd"));
            //    var days = ts.Days;
            //    if (hour >=12)//
            //    {
            //        if (days <3)
            //        {
            //            str = "服务器时间已过12点，预约款项请提前三天办理。";
            //            resultList.Add(new ValidationResult("", str));
            //            return resultList;
            //        }
            //    }
