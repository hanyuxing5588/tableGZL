...
namespace Business.Reimbursement
{
    ...
    public class 现金报销单 : BXDocument
    {

        public 现金报销单() : base() { }
        public 现金报销单(Guid OperatorId ,string ModelUrl) : base(OperatorId, ModelUrl) { }
                ...
                if (main != null)
                {
                    ...
                    if (details.Count > 0)
                    {
                        ...
                        foreach (BX_DetailView detail in details)
                        {
                            ...
                            ////科目摘要信息
                            //SS_BGCodeMemoView bgMemo = this.InfrastructureContext.SS_BGCodeMemoView.FirstOrDefault(e => e.BGCodeMemo == detail.FeeMemo);
                            //if (bgMemo != null)
                            //{
                            //    picker.AddRange(bgMemo.Pick());
                            //}
                            jgm.r.Add(picker);
                        }
                    }
        ...
        private void AddDetail(BX_Main main,List<JsonAttributeModel> row,int orderNum)
        {
            ...
            //////科目摘要信息
            ////SS_BGCodeMemo bgmemo = new SS_BGCodeMemo();
            ////bgmemo.Fill(row);
            ////if (bgmemo != null && !string.IsNullOrEmpty(bgmemo.BGCodeMemo))
            ////{
            ////    temp.FeeMemo = bgmemo.BGCodeMemo;
            ////}
            ...
            else
            {
                ...
                foreach (BX_Detail detail in detailList)
                {
                    ...
                    else
                    {
                        ...
                        //if (bgmemo != null && !string.IsNullOrEmpty(bgmemo.BGCodeMemo))
                        //{
                        //    detail.FeeMemo = bgmemo.BGCodeMemo;
                        //}
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(strMsg))
                {
                    ...
                    OperatorLog.WriteLog(this.OperatorId, value, status, "现金报销单", data);
            ...
            {
                OperatorLog.WriteLog(this.OperatorId, "现金报销单", ex.Message, data, false);
                result.result = JsonModelConstant.Error;
        ...
        /// <returns></returns>
        private BX_Main LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultDetail(BX_Main data)
        {
        ...
        protected override VerifyResult DeleteVerify(Guid guid)
        {
            ...
            var hxDetail = this.BusinessContext.HX_Detail.FirstOrDefault(e => e.GUID_Main == guid);
            if (hxDetail != null) {
                str = "此报销单已经核销不能删除！";
                resultList.Add(new ValidationResult("", str));
                result._validation = resultList;
        ...
        protected override VerifyResult ModifyVerify(object data)
        {
            ...
            if (WorkFlowAPI.ExistProcessCurPerson(model.GUID,this.OperatorId))
            {
                List<ValidationResult> resultList = new List<ValidationResult>();
