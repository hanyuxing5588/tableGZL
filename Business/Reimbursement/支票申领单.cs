...
{   
    public class 支票申领单 : BXDocument
    {

        public 支票申领单() : base() { }
        public 支票申领单(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel Retrieve(Guid guid)
        {
            ...
            try
            {
                ...
                if (main != null)
                {
                    ...
                    IQueryable<BX_DetailView> q = this.BusinessContext.BX_DetailView.Where(e => e.GUID_BX_Main == guid).OrderBy(e => e.OrderNum);
                    List<BX_DetailView> details = q == null ? new List<BX_DetailView>() : q.ToList();
                    if (details.Count > 0)
                    {
                    ...
                    BX_DetailView dModel = new BX_DetailView();
                    dModel.FillDetailDefault<BX_DetailView>(this, this.OperatorId);
                    JsonGridModel fjgm = new JsonGridModel(dModel.ModelName());
                    jmodel.f.Add(fjgm);
        ...
        {
            BX_Main main = this.BusinessContext.BX_Main.Include("BX_Detail").FirstOrDefault(e => e.GUID == guid);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            BX_Main main = new BX_Main(); BX_Detail tempdetail = new BX_Detail();
            JsonAttributeModel id = jsonModel.m.IdAttribute(main.ModelName());
            if (id == null) return Guid.Empty;
            Guid g;
            if (Guid.TryParse(id.v, out g) == false) return Guid.Empty;
            main = this.BusinessContext.BX_Main.Include("BX_Detail").FirstOrDefault(e => e.GUID == g);
            if (main != null)
            {
            ...
            if (Grid == null)
            {
                foreach (BX_Detail detail in main.BX_Detail) { this.BusinessContext.DeleteConfirm(detail); }
            }
            else
            {
                ...
                foreach (BX_Detail detail in detailList)
                {
                    ...
                    else
                    {
                        ...
                        //detail.CN_PaymentNumber = newnumber;
                        ...
                        //this.BusinessContext.ModifyConfirm(newnumber);
                ...
                {
                    orderNum++;
                    BX_Detail newitem = new BX_Detail();
                    ...
                    newitem.CN_PaymentNumber.GUID_Project = newitem.GUID_Project;
                    ...
                    main.BX_Detail.Add(newitem);

                    //this.BusinessContext.InsertConfirm(newitem);
                }
            }
            this.BusinessContext.ModifyConfirm(main);
            ...
            if (jsonModel.d != null && jsonModel.d.Count > 0)
            {
                ...
                if (Grid != null)
                {
                    ...
                    foreach (List<JsonAttributeModel> row in Grid.r)
                    {
                        ...
                        temp.GUID_BX_Main = main.GUID;
                        temp.GUID_PaymentNumber = temp.CN_PaymentNumber.GUID;
                        temp.Total_BX = temp.Total_Real;
                        main.BX_Detail.Add(temp);
                    }
                }
            }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(strMsg))
                {
                    ...
                    OperatorLog.WriteLog(this.OperatorId, value, status, "支票申领单", data);
            ...
            {
                OperatorLog.WriteLog(this.OperatorId, "支票申领单", ex.Message, data, false);
                result.result = JsonModelConstant.Error;
        ...
        private List<ValidationResult> VerifyResult_Bx_Detail(BX_Detail data, int rowIndex)
        {
            ...
            //限额和报销金额（实领金额）任意一个存在（或两者都存在）即可
            if (item.Total_Plan == 0F && item.Total_Real == 0F)
            {
                str = "金额填写不正确!";
                resultList.Add(new ValidationResult("第" + rowIndex.ToString() + "行", str));
            }
            ...
            ////限额
            //if (item.Total_Plan.IsNullOrEmpty() && Common.ConvertFunction.TryParse(item.Total_Plan.GetType(), item.Total_Plan.ToString(), out g) == false)
            //{
            //    str = "明细限额格式不正确！";
            //    resultList.Add(new ValidationResult("第" + rowIndex.ToString() + "行", str));
            ...
            //收款单位
            if (!item.GUID_Cutomer.IsNullOrEmpty() && Common.ConvertFunction.TryParse(item.GUID_Cutomer.GetType(), item.GUID_Cutomer.ToString(), out g) == false)
            {
                str = "收款单位 格式不正确！";
                resultList.Add(new ValidationResult("第" + rowIndex.ToString() + "行", str));
            }
        ...
        protected override VerifyResult InsertVerify(object data)
        {
            ...
            else
            {
                ...
                list.Add(new ValidationResult("", "单据体中至少有一行记录才能保存，请处理后再保存！"));
