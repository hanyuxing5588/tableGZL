...
namespace Business.Accountant
{
    ...
    select GUID_InvitePerson,sum(Total_BX) as Total_BX,sum(Total_Tax) as Total_Tax,sum(Total_Real) as Total_Real 
    from bx_InviteFeeView  
    left join bx_main main on main.guid=bx_InviteFeeView.GUID_BX_Main 
    where main.guid in (
    ...
    and InvitePersonIDCard in (select isnull(IDCard,'''') from ss_person) group by GUID_InvitePerson
    ...
    and InvitePersonIDCard in (select isnull(IDCard,'''') from ss_person)
  */
    public class 单位人员领款单 : BaseDocument
    {

        public 单位人员领款单() : base() { }
        public 单位人员领款单(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        /// <summary>
        /// 单位人员领款单（汇总）
        /// </summary>
        ...
        public List<lwfgsydhzModel> MX_List(SearchCondition conditions, out string message)
        {
            ...
            where main.guid in (
              ...
              and InvitePersonIDCard in (select isnull(IDCard,'''') from ss_person) and invitepersonname like '%%'
