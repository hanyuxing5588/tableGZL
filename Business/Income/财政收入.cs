...
using BusinessModel;
namespace Business.Income
{    
    public class 财政收入 : BaseDocument
    {

        public 财政收入() : base() { }
        public 财政收入(Guid OperatorId, string ModelUrl) : base(OperatorId, ModelUrl) { }
        ...
        public override JsonModel New()
        {
            try
            {
                ...
                SR_MainView model = new SR_MainView();
                model.FillDefault(this, this.OperatorId);
                jmodel.m = model.Pick();

                SR_DetailView dModel = new SR_DetailView();
                dModel.FillDetailDefault<SR_DetailView>(this, this.OperatorId);
        ...
        public override JsonModel Retrieve(Guid guid)
        {
            ...
            try
            {
                ...
                if (main != null)
                {
                    ...
                    IQueryable<SR_DetailView> q = this.BusinessContext.SR_DetailView.Where(e => e.GUID_SR_Main == guid).OrderBy(e => e.OrderNum);
                    List<SR_DetailView> details = q == null ? new List<SR_DetailView>() : q.ToList();
                    if (details.Count > 0)
                    {
                        JsonGridModel jgm = new JsonGridModel(details[0].ModelName());
                        jmodel.d.Add(jgm);
                        foreach (SR_DetailView detail in details)
                        {
        ...
        protected override Guid Insert(JsonModel jsonModel)
        {
            ...
            {
                SR_DetailView temp = new SR_DetailView();
            ...
            this.BusinessContext.SR_Main.AddObject(main);
        ...
        private void AddDetail(SR_Main main, List<JsonAttributeModel> row, int orderNum)
        {

            orderNum++;
            SR_Detail temp = new SR_Detail();
            ...
            temp.GUID_SR_Main = main.GUID;
            temp.GUID_PaymentNumber = temp.CN_PaymentNumber.GUID;
            main.SR_Detail.Add(temp);
        ...
        protected override Guid Modify(JsonModel jsonModel)
        {
            ...
            if (jsonModel.m == null) return Guid.Empty;
            SR_Main main = new SR_Main(); SR_Detail tempdetail = new SR_Detail();
            JsonAttributeModel id = jsonModel.m.IdAttribute(main.ModelName());
            if (id == null) return Guid.Empty;
            Guid g;
            if (Guid.TryParse(id.v, out g) == false) return Guid.Empty;
            main = this.BusinessContext.SR_Main.Include("SR_Detail").FirstOrDefault(e => e.GUID == g);
            if (main != null)
            {
            ...
            if (Grid == null)
            {
                foreach (SR_Detail detail in main.SR_Detail) { this.BusinessContext.DeleteConfirm(detail); }
            }
            else
            {
                List<SR_Detail> detailList = new List<SR_Detail>();
                foreach (SR_Detail detail in main.SR_Detail)
                {
                    detailList.Add(detail);
                }
                var orderNum = 0;
                foreach (SR_Detail detail in detailList)
                {
        ...
        {
            SR_Main main = this.BusinessContext.SR_Main.Include("SR_Detail").FirstOrDefault(e => e.GUID == guid);

            List<SR_Detail> details = new List<SR_Detail>();

            foreach (SR_Detail item in main.SR_Detail)
            {
                details.Add(item);
            }

            foreach (SR_Detail item in details) { BusinessContext.DeleteConfirm(item); }
        ...
        public override JsonModel Save(string status, JsonModel jsonModel)
        {
            ...
            try
            {
                Guid value = jsonModel.m.Id(new SR_Main().ModelName());
                ...
                if (string.IsNullOrEmpty(strMsg))
                {
                    ...
                    OperatorLog.WriteLog(this.OperatorId, value, status, "财政收入", data);
            ...
            {
                OperatorLog.WriteLog(this.OperatorId, "财政收入", ex.Message, data, false);
                result.result = JsonModelConstant.Error;
        ...
        {
            SR_Main main = this.BusinessContext.SR_Main.FirstOrDefault(e => e.GUID == guid);
            if (main != null)
            {
        ...
        /// <returns></returns>
        private SR_Main LoadMain(JsonModel jsonModel)
        {
        ...
        /// <returns></returns>
        private List<ValidationResult> VerifyResultDetail(SR_Main data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            //明细验证
            List<SR_Detail> detailList = new List<SR_Detail>();
            foreach (SR_Detail item in data.SR_Detail)
            {
                detailList.Add(item);
            }
            ...
            {
                var rowIndex = 0;
                foreach (SR_Detail item in detailList)
                {
                    rowIndex++;
                    var vf_detail = VerifyResult_SR_Detail(item, rowIndex);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        private List<ValidationResult> VerifyResultMain(SR_Main data)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            SR_Main mModel = data;
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        private List<ValidationResult> VerifyResult_SR_Detail(SR_Detail data, int rowIndex)
        {
            string str = string.Empty;
            List<ValidationResult> resultList = new List<ValidationResult>();
            object g;
            ...
            {
                if (Common.ConvertFunction.TryParse(item.Total_SR.GetType(), item.Total_SR.ToString(), out g) == false)
                {
                    str = "明细金额格式不正确！";
                    resultList.Add(new ValidationResult("第" + rowIndex.ToString() + "行", str));
        ...
        protected override VerifyResult ModifyVerify(object data)
        {
            ...
            VerifyResult result = new VerifyResult();
            SR_Main model = (SR_Main)data;
            SR_Main orgModel = this.BusinessContext.SR_Main.Include("SR_Detail").FirstOrDefault(e => e.GUID == model.GUID);
            if (orgModel != null)
            {
