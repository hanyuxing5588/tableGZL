    ...
    public class 预算编制 : BaseDocument
    {
        ...
        protected override VerifyResult ModifyVerify(object data)
        {
            ...
            if (WorkFlowAPI.ExistProcess(model.GUID))
            {
                ...
                FlowNodeModel objFnm = WorkFlowAPI.GetCurNodeByDocId((Guid)model.GUID_BG_Assign, out strErr);
                if (null == objFnm)
                {
                    List<ValidationResult> resultList = new List<ValidationResult>();
                ...
                {
                    SS_RunTimeUsersSet objRTUS = this.BusinessContext.SS_RunTimeUsersSet.FirstOrDefault(e => e.WorkFlowNodeId == objFnm.WorkFlowNodeId);
                    if (null == objRTUS)
                    {
                        List<ValidationResult> resultList = new List<ValidationResult>();
                    ...
                    else if (objRTUS.OperatorId != this.OperatorId)
                    {
                        List<ValidationResult> resultList = new List<ValidationResult>();
